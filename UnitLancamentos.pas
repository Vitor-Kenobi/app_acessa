...
begin
    ...
    begin
      if x = 1 then
          FrmPrincipal.AddLancamento(FrmLancamentos.lv_lancamento,'01', 'Biologia', 'Ciências Biológicas', '2', date, foto);

      if x = 2 then
         FrmPrincipal.AddLancamento(FrmLancamentos.lv_lancamento,'02', 'História', 'Ciências Humanas', '1', date, foto);

      if x = 3 then
         FrmPrincipal.AddLancamento(FrmLancamentos.lv_lancamento,'03', 'Português', 'Línguas', '1', date, foto);

      if x = 4 then
      begin
         FrmPrincipal.AddLancamento(FrmLancamentos.lv_lancamento,'04', 'Matemática', 'Ciências Exatas', '1', date, foto);
      end
      else
    ...
    lbl_count_registros.Text := IntToStr(itens) + ' matéria(s)';
